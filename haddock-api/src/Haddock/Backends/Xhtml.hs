...
import Data.Maybe
import Data.Char (isLetter, isAscii, isSpace, toUpper)
import Data.List (sortBy, nubBy, intersperse, intercalate, isPrefixOf)
...
ppHtmlContents dflags odir doctitle maybePackage themes
  ...
  let tree = mkModuleTree dflags showPkgs
      ...
      sig_tree = mkModuleTree dflags showPkgs
                 ...
                 | iface <- ifaces
                 , instIsSig iface ]
      instIsSig = const False -- FIXME: signatures are not yet here
      html =
        headHtml doctitle themes maybeMathJaxUrl +++
...
ppHtmlModule odir doctitle maybePackage themes instIfaces
  ...
  let sigDoc    = H.sup << ("[" +++ H.anchor ! [H.href sigDocURL] << "?" +++ "]")
  let ifaceIsSig = const False -- FIXME: signatures are not yet here
...
genModuleInfo iface
  ...
  where
    ...
    multilineRow title xs = let toLines = mconcat . intersperse H.br . map H.toHtml
                            in (H.th ! [H.valign "top"]) << title <-> H.td << (toLines xs)
...
processExport summary links unicode qual item = case item of
  ...
  ExportDecl decl doc subdocs insts fixities splice ->
    processDecl summary (
      ppDecl summary links decl doc insts fixities subdocs splice unicode qual
    )
  ExportNoDecl y [] ->
